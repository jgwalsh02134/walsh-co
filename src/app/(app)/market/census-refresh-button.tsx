...
/**
 * Refresh trigger for Census ACS demographics context. The API key
 * never reaches the client — only the boolean `keyConfigured`.
 */
export function CensusRefreshButton({
  keyConfigured,
}: {
  keyConfigured: boolean;
}) {
  const [state, action, pending] = useActionState<
    CensusAcsRefreshResult | null,
    FormData
    ...
    return (
      <div className="flex flex-col gap-1 text-right">
        <button
          ...
          disabled
          aria-disabled
          title="Set CENSUS_API_KEY on the server to enable refresh."
  ...
  return (
    <form action={action} className="flex flex-col gap-1">
      ...
      >
        {pending ? "Refreshing…" : "Refresh Census ACS context"}
      </button>
...
function RefreshSummary({ result }: { result: CensusAcsRefreshResult }) {
  const tone =
    result.status === "ok"
