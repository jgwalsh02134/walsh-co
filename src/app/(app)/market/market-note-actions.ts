...
const NO_PARROT_CLAUSE = [
  "STRICT RULE — DO NOT PARROT THE DASHBOARD.",
  "The dashboard already displays provider estimates and source values from RentCast, ATTOM, Zillow ZHVI, FRED, Google Maps, Census, and manual entries.",
  "Do not restate them unless needed to explain a conflict.",
  "Focus on what is not obvious from the dashboard.",
  ...
  "VIOLATION EXAMPLE: \"RentCast estimates the home value at $X with rent at $Y\" — this is a parrot of the dashboard and is forbidden.",
  "ACCEPTABLE EXAMPLE: \"FHFA HPI for this metro implies +5% YoY, while the dashboard's ZIP trend shows +1% — investigate the divergence.\" — this is allowed because it adds external context and flags a conflict.",
].join(" ");
...
export async function generateMarketNote(
  ...
  const prompt = [
    ...
    "The dashboard already displays provider estimates and source values. Do not restate them unless needed to explain a conflict. Focus on what is not obvious from the dashboard.",
...
export async function generatePropertyAnalysis(
  ...
  const prompt = [
    ...
    SAFETY_CLAUSE,
    "The dashboard already displays provider estimates and source values for this property. Do not restate them unless needed to explain a conflict. Focus on what is not obvious from the dashboard.",
...
export async function generatePropertyAnalysisWithWebSearch(
  ...
  const prompt = [
    ...
    "## External Signal",
    "- New external context only. Note staleness when applicable.",
    "## Confirmation / Conflict",
    "- Where external evidence supports or conflicts with the dashboard.",
    "## Missing Facts",
    "- Property-specific gaps (public records, condition, permits, rent comps, etc.).",
    "## Next Checks",
    "- 3–5 concise actions.",
    "Do NOT include a Markdown ## Sources section — the UI renders sources separately.",
    "",
