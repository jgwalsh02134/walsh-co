...
/**
 * Client-side test buttons + result panel for the xAI / Grok integration.
 *
 * Mirrors OpenAITestPanel for visual and behavioral parity.
 */
export function XaiTestPanel({ configured }: { configured: boolean }) {
  const [state, setState] = useState<XaiTestState | null>(null);
  ...
  const run = (kind: "normal" | "search") => {
    ...
    startTransition(async () => {
      const result =
        kind === "normal" ? await testXaiConnection() : await testXaiWebSearch();
      setState(result);
    });
  };
